...
public class MoveToGoal : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Forward ray (e.g., for wall detection)
        ...
        // New low bar detection: forward + upward diagonal ray
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        int squish = actions.DiscreteActions[2];

        float horizontal = 0f;
        if (move == 1) horizontal = moveSpeed;
        else if (move == 2) horizontal = -moveSpeed;

        body.linearVelocity = new Vector2(horizontal, body.linearVelocity.y);

        if (jump == 1 && IsGrounded() && transform.localScale.y >= 0.99f)
        {
        ...
        // Reward for reaching distance checkpoints
